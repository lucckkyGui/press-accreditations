...
export const eventService = {
  ...
  async getEvents(params?: EventsQueryParams): Promise<ApiResponse<Event[]>> {
    try {
      ...
      return {
        data: data.map(mapDbEventToEvent),
        pagination: count ? {
          total: count,
          page: params?.page || 0,
  ...
  async createEvent(event: Partial<Event>): Promise<ApiResponse<Event>> {
    try {
      const { data, error } = await supabase
        .from('events')
        .insert([mapEventToDbEvent(event)])
        .select()
        .single();
  ...
  async updateEvent(id: string, event: Partial<Event>): Promise<ApiResponse<Event>> {
    try {
      const { data, error } = await supabase
        .from('events')
        .update(mapEventToDbEvent(event))
        .eq('id', id)
        .select()
        .single();
...
function mapDbEventToEvent(dbEvent: EventDB): Event {
  return {
    ...
    endDate: dbEvent.endDate ? new Date(dbEvent.endDate) : undefined,
    organizerId: dbEvent.organizerId || "",
    isPublished: dbEvent.isPublished || false,
...
function mapEventToDbEvent(event: Partial<Event>): Partial<EventDB> {
  ...
  return Object.fromEntries(
    Object.entries(dbEvent).filter(([_, v]) => v !== undefined)
  );
}
